...
function open_wsdl(uri, options, callback) {
  ...
  if (/^https?:/.test(uri)) {
    ...
    localuri = process.cwd() + '/cache/' + uri.replace('http://','');
    folder = path.dirname(localuri);
  ...
  if (!/^https?:/.test(uri)) {
    var fileToRead = uri;
    if(fromFile && localuri){
      fileToRead = localuri;
    }
    debug('Reading file: %s', uri);
    fs.readFile(fileToRead, 'utf8', function(err, definition) {
      if (err) {
        callback(err);
      }
      else {
    ...
    httpClient.request(uri, null /* options */, function(err, response, definition) {
      fs.writeFile(localuri,definition);
      if (err) {
        callback(err);
